...
# RAG mode: set to "tfidf" for zero-download hackathon reliability
export RAG_EMB=${RAG_EMB:-tfidf}

# Gemini
if [ -z "$GEMINI_API_KEY" ]; then
  echo "[warn] GEMINI_API_KEY is not set. Summary/QA calls will fail."
fi

uvicorn server.app.main:app --host 0.0.0.0 --port "$PORT" --reload
